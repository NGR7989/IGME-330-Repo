...
const showItems = () => {
  ...
  let newArr = items.map((x) => `<li> ${x} </li>`).join("");
  results.innerHTML = newArr;
...
document.getElementById("btn-add").onclick = () => {
  ...
  const text = document.getElementById("thing-text");
  let str = text.value;
  text.value = "";

  items.push(str);
  storage.writeToLocalStorage("ngr7989", items);
...
items = storage.readFromLocalStorage("ngr7989");
showItems();
